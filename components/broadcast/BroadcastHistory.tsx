...
const RECIPIENT_STATUS_STYLES: Record<string, string> = {
  ...
  sent: 'text-green-400',
  failed: 'text-red-400',
  insufficient_credit: 'text-amber-400',
}

export default function BroadcastHistory({ clientId, refreshKey }: { clientId: string; refreshKey: number }) {
  const [broadcasts, setBroadcasts] = useState<BroadcastRow[]>([])
  const [loading, setLoading] = useState(true)
  const [activeId, setActiveId] = useState<string | null>(null)
  const [detail, setDetail] = useState<{ broadcast: BroadcastRow; recipients: RecipientRow[] } | null>(null)
  const [detailLoading, setDetailLoading] = useState(false)

  function load() {
    setLoading(true)
    fetch(`/api/broadcasts?clientId=${clientId}`)
      .then((r) => (r.ok ? r.json() : []))
      .then(setBroadcasts)
      .catch(() => {})
  ...
  function openDetail(id: string) {
    ...
    fetch(`/api/broadcasts/${id}?clientId=${clientId}`)
  ...
  if (broadcasts.length === 0) {
    return <p className="text-sm text-muted">No broadcasts sent yet for this institute.</p>
  }
        ...
        <tbody>
          {broadcasts.map((b) => (
            ...
            >
              ...
              <td className="py-2 pr-2 font-mono text-xs text-muted2">{b.template_name}</td>
              ...
              <td className="py-2 pr-2 text-xs text-muted2">
                ...
                {b.insufficient_credit_count > 0 && (
                  <span className="ml-1 text-amber-400">({b.insufficient_credit_count} no credit)</span>
                )}
              </td>
            ...
            {detailLoading || !detail ? (
              ...
              <>
                <p className="mb-4 text-sm text-muted2">
                  Template <span className="font-mono text-fg">{detail.broadcast.template_name}</span> —{' '}
                  {detail.broadcast.sent_count} sent, {detail.broadcast.failed_count} failed,{' '}
                  {detail.broadcast.insufficient_credit_count} blocked by wallet, out of{' '}
                  {detail.broadcast.total_recipients} total.
                </p>
                    ...
                    {detail.recipients.map((r) => (
                      <tr key={r.lead_id} className="border-b border-border/50 last:border-0">
                        <td className="py-2 pr-2 text-xs text-fg">
                          {r.full_name}
                          <div className="text-muted2">{r.phone_number}</div>
                        </td>
