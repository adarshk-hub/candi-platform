          ...
          {assignmentsLoading ? (
            ...
            <div className="space-y-2">
              {SEQUENCE_STEPS.map((def, i) => {
                ...
                const currentApproved = !!current && approvedTemplates.some((t) => t.name === current)
                  ...
                  <div key={def.day} className="rounded-md border border-border bg-card2 px-3 py-2">
                    <div className="flex flex-wrap items-center gap-3">
                      ...
                      <select
                        ...
                        title={currentApproved ? 'Only send this step to leads in this stage' : 'Pick an approved template first'}
                      ...
                      {savingDay !== def.day && msg && (
                        <span className={`shrink-0 text-[11px] ${msg.ok ? 'text-green-500' : 'text-red-400'}`}>
                          {msg.ok ? '✓ Saved' : ''}
                        </span>
                      )}
                    </div>
                    ...
                    {current && !currentApproved && (
                      <p className="mt-1.5 text-[11px] text-amber-500">
                        "{current}" is not an approved template, so this step will fail to send. Pick an approved
                        template above — stage and Ask first can be changed after that.
                      </p>
                    )}

                    {/* A step with no stage and "Ask first" ticked is never
                        sent by anything: the timer skips ask-first steps and
                        the stage-change prompt only looks for a matching
                        stage. Say so instead of letting it look configured. */}
                    {i > 0 && currentApproved && !stage && !!confirmByDay[def.day] && (
                      <p className="mt-1.5 text-[11px] text-amber-500">
                        With "Any stage" and Ask first ticked, this message will never be sent. Choose a stage to be
                        asked on, or untick Ask first to send it on day {def.day}.
                      </p>
                    )}
                  </div>
                )
