...
// This is central-registry data (the list of clients itself), not any one
// client's own CRM data, so it uses centralQuery() rather than query().
export async function GET(req: NextRequest) {
  const session = getSession(req)
  if (!session) return NextResponse.json({ error: 'Unauthorized' }, { status: 401 })
  if (!AGENCY_ROLES.includes(session.role)) return NextResponse.json([])

  const rows = await centralQuery('SELECT id, name FROM clients ORDER BY name')
