...
import { pauseSequence } from '@/lib/waSequenceEngine'
...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const ok = await pauseSequence(params.id, body?.note)
  if (!ok) return NextResponse.json({ error: 'Sequence is not active' }, { status: 400 })
